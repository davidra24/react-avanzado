define("./workbox-d41f36cc.js",['exports'], function (exports) { 'use strict';
    ...
    */
    ...
    const _getEffectiveRequest$1 = async ({
      request,
      mode,
      plugins = []
    }) => {
      const cacheKeyWillBeUsedPlugins = pluginUtils$1.filter(plugins, "cacheKeyWillBeUsed"
      /* CACHE_KEY_WILL_BE_USED */
      );
      let effectiveRequest = request;
    ...
    const _isResponseSafeToCache$1 = async ({
      request,
      response,
      event,
      plugins = []
    ...
    const matchWrapper$1 = async ({
      ...
      const cache = await self.caches.open(cacheName);
      const effectiveRequest = await _getEffectiveRequest$1({
        plugins,
        request,
        mode: 'read'
      });
    ...
    const putWrapper$1 = async ({
      ...
      const responseToCache = await _isResponseSafeToCache$1({
        event,
        plugins,
        response,
        request: effectiveRequest
      ...
      const cache = await self.caches.open(cacheName);
      const updatePlugins = pluginUtils$1.filter(plugins, "cacheDidUpdate"
      /* CACHE_DID_UPDATE */
      );
      const oldResponse = updatePlugins.length > 0 ? await matchWrapper$1({
        cacheName,
        matchOptions,
        request: effectiveRequest
      }) : null;
    ...
    const cacheWrapper$1 = {
      put: putWrapper$1,
      match: matchWrapper$1
    };
    ...
    */
    ...
    const wrappedFetch$1 = async ({
      ...
      const failedFetchPlugins = pluginUtils$1.filter(plugins, "fetchDidFail"
      /* FETCH_DID_FAIL */
      ); // If there is a fetchDidFail plugin, we need to save a clone of the
      // original request before it's either modified by a requestWillFetch
      // plugin or before the original request's body is consumed via fetch().
    ...
    */
    ...
    function createCacheKey(entry) {
      if (!entry) {
        throw new WorkboxError$2('add-to-cache-list-unexpected-type', {
          entry
        });
    ...
    */
    ...
    const logGroup = (groupTitle, deletedURLs) => {
      logger$2.groupCollapsed(groupTitle);
    ...
    function printCleanupDetails(deletedURLs) {
      ...
      if (deletionCount > 0) {
        logger$2.groupCollapsed(`During precaching cleanup, ` + `${deletionCount} cached ` + `request${deletionCount === 1 ? ' was' : 's were'} deleted.`);
    ...
    */
    ...
    function printInstallDetails(urlsToPrecache, urlsAlreadyPrecached) {
      ...
      if (precachedCount || alreadyPrecachedCount) {
        ...
        logger$2.groupCollapsed(message);
    ...
    */
    ...
    class PrecacheController {
      ...
      constructor(cacheName) {
        this._cacheName = cacheNames$1.getPrecacheName(cacheName);
      ...
      addToCacheList(entries) {
        {
          finalAssertExports$2.isArray(entries, {
        ...
        for (const entry of entries) {
          ...
          if (this._urlsToCacheKeys.has(url) && this._urlsToCacheKeys.get(url) !== cacheKey) {
            throw new WorkboxError$2('add-to-cache-list-conflicting-entries', {
          ...
          if (typeof entry !== 'string' && entry.integrity) {
            if (this._cacheKeysToIntegrities.has(cacheKey) && this._cacheKeysToIntegrities.get(cacheKey) !== entry.integrity) {
              throw new WorkboxError$2('add-to-cache-list-conflicting-integrities', {
                url
              });
            }
      ...
      async install({
        ...
        {
          if (plugins) {
            finalAssertExports$2.isArray(plugins, {
              moduleName: 'workbox-precaching',
              className: 'PrecacheController',
              funcName: 'install',
              paramName: 'plugins'
      ...
      async _addURLToCache({
        ...
        let response = await fetchWrapper$1.fetch({
          event,
          plugins,
          request
        ...
        if (!isValidResponse) {
          throw new WorkboxError$2('bad-precaching-response', {
            url,
      ...
      createHandler(fallbackToNetwork = true) {
        return async ({
          ...
          try {
            ...
            throw new WorkboxError$2('missing-precache-entry', {
              cacheName: this._cacheName,
            ...
            if (fallbackToNetwork) {
              {
                logger$2.debug(`Unable to respond with precached response. ` + `Falling back to network.`, error);
              }
      ...
      createHandlerBoundToURL(url, fallbackToNetwork = true) {
        ...
        if (!cacheKey) {
          throw new WorkboxError$2('non-precached-url', {
            url
          });
        }
    ...
    */
    ...
    const addFetchListener = ({
      ...
      const cacheName = cacheNames$1.getPrecacheName(); // See https://github.com/Microsoft/TypeScript/issues/28357#issuecomment-436484705
      ...
      self.addEventListener('fetch', event => {
        ...
        if (!precachedURL) {
          {
            logger$2.debug(`Precaching did not find a match for ` + getFriendlyURL$2(event.request.url));
          }
        ...
        let responsePromise = self.caches.open(cacheName).then(cache => {
          ...
          {
            logger$2.warn(`The precached response for ` + `${getFriendlyURL$2(precachedURL)} in ${cacheName} was not found. ` + `Falling back to the network instead.`);
          }
...

        {
          responsePromise = responsePromise.then(response => {
            ...
            // print the routing details to the console.
            logger$2.groupCollapsed(`Precaching is responding to: ` + getFriendlyURL$2(event.request.url));
            logger$2.log(`Serving the precached url: ${precachedURL}`);
            logger$2.groupCollapsed(`View request details here.`);
            logger$2.log(event.request);
            logger$2.groupEnd();
            logger$2.groupCollapsed(`View response details here.`);
    ...
    */
    ...
    const installListener = event => {
      ...
      event.waitUntil(precacheController.install({
        ...
        {
          logger$2.error(`Service worker installation failed. It will ` + `be retried automatically during the next navigation.`);
...
//# sourceMappingURL=workbox-d41f36cc.js.map
